...
public static class IdentityEndpoints
{
    ...
    public static void RegisterIdentityEndpoints(this WebApplication app)
    {
        ...
        auth.MapPost("login",
            ...
            {
                var user = await authenticationService.Login(request.Username, request.Password,
                    rememberMe); // Login user
                if (user is not null)
                {
                    return TypedResults.Ok("Successfully logged in");
                }

                return (IResult)TypedResults.BadRequest("Username or password is incorrect");
            });
